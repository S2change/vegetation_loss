...
1. **before**: multiband geotiff file (`int16`) which is a time composite *before* date *D*, with *N* bands. Band values are reflectances between 0 and 10000. There are no NoData values due to compositing (if no value is available, pixel value is 0).
