...
* [Apresentação](documents/presentations/reuniao_DGT_4_julho_2024.pdf)
...
entregáveis:
  ...
  * E3.2 – Demonstrador prático: mapas nacionais vetoriais a delimitar manchas de perda recente de floresta e mato superiores a 0.5 ha com uma frequência bimestral relativos a um período contínuo de dois anos entre 2023 e 2025. Mapas vectoriais bimestrais para os anos 2023 e 2024 foram elaborados como resultado de processamento de séries temporais Sentinel-2 através do algoritmo CCD para Portugal Continental e para a máscara de potenciais perdas de vegetação fornecida pela DGT (aprox. 500 M pixels Sentinel-2 de resolução 10 m). O processamento foi realizado em ambiente HPC usando a plataforma INCD/CNCA/LIP. Os mapas (em formato shapefile) estão disponível neste [link] (requer password).
  * E3.3 – Relatório de validação dos mapas nacionais. Foram feitos dois exercícios de validação:
    - Usando [BDR_DGT_300](data_info/reference_data/BDR_DGT_300) e os mapas bimestrais de perdas de vegetação. Ver [script](scripts/validation/raster_avaliacao_exatidao.py) e relatório.
    - Fazendo uma comparação para os mapas bimestrais de perdas de vegetação para 2023-2024 com as áreas ardidas ICNF para Portugal Continental. Ver relatório.
  * E3.4 – Aplicação informática que possa ser integrada na cadeia de produção da DGT. Os scripts (que foram desenvolvidos para serem compatíveis com o pipeline de processamento de dados Sentinel-2 em ambiente HPC) estão disponíveis neste repositório. Em particular, foi criado e testado o código para as seguintes tarefas.
