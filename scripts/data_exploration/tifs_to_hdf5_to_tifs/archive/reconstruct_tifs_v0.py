...
h5_filename = r'C:\Users\mlc\OneDrive - Universidade de Lisboa\Documents\temp\test_tif_to_hdf5\test_1667647823345_6bands.h5' #T29TNE_6bands.h5'
...
with h5py.File(h5_filename, 'r') as h5f:
    ...
    print(f"Shape of values dataset: {h5f['values'].shape}, expected: (num_timestamps, num_bands, num_pixels)")
    ...
    print(f"Grid dimensions: width={width}, height={height}")
    # compare with range of xs and ys to confirm they match
    print(f"Range of xs: {unique_xs[0]} to {unique_xs[-1]}, difference: {unique_xs[-1] - unique_xs[0]}, expected range based on width: {unique_xs[0]} to {unique_xs[0] + (width - 1) * (unique_xs[1] - unique_xs[0])}")
    print(f"Range of ys: {unique_ys[-1]} to {unique_ys[0]}, difference: {unique_ys[0] - unique_ys[-1]}, expected range based on height: {unique_ys[0]} to {unique_ys[0] - (height - 1) * (unique_ys[0] - unique_ys[1])}")       
    ...
    for i in range(10): #len(ts_ordinals)):
