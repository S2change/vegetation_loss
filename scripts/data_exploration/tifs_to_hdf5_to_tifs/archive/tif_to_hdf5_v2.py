...
# supposing 4band and 2band are in separate folders but have the same filenames (except for the band differences), we will use the 4band folder as the reference for sorting and metadata extraction, and then check spatial alignment with the 2band files to ensure they match spatially. We will then extract the pixel values from both sets of files for the common intersection area and write them to the HDF5 file.
...
# if 4bands and 2bands are in distinct folders but have the same filenames (except for the band differences), we can use the 4band folder as the reference for sorting and metadata extraction, and then check spatial alignment with the 2band files to ensure they match spatially. We will then extract the pixel values from both sets of files for the common intersection area and write them to the HDF5 file.
...
if False:
    folder_path_4bands = r"H:\s2_images\T29TNE"
    folder_path_2bands = r"D:\s2_images_B2_B11\T29TNE"
    h5_filename = os.path.join(folder_path, 'T29TNE_6bands.h5') # output in folder_path (to be changed)
...
# describe min_xs, max_xs, min_ys, max_ys to understand the variability of extents across files, including q1 and q3 to understand the interquartile range and identify potential outliers in the extents. This will help us choose an appropriate tol value for filtering.
print(f"Extent variability across files:")  
print(f"  - min_x: median={np.median(min_xs):.1f}, min={min_xs.min():.1f}, q1={np.percentile(min_xs, 25):.1f}, q3={np.percentile(min_xs, 75):.1f}, max={min_xs.max():.1f}, std={min_xs.std():.1f}")
print(f"  - max_x: median={np.median(max_xs):.1f}, min={max_xs.min():.1f}, q1={np.percentile(max_xs, 25):.1f}, q3={np.percentile(max_xs, 75):.1f}, max={max_xs.max():.1f}, std={max_xs.std():.1f}")
print(f"  - min_y: median={np.median(min_ys):.1f}, min={min_ys.min():.1f}, q1={np.percentile(min_ys, 25):.1f}, q3={np.percentile(min_ys, 75):.1f}, max={min_ys.max():.1f}, std={min_ys.std():.1f}")
print(f"  - max_y: median={np.median(max_ys):.1f}, min={max_ys.min():.1f}, q1={np.percentile(max_ys, 25):.1f}, q3={np.percentile(max_ys, 75):.1f}, max={max_ys.max():.1f}, std={max_ys.std():.1f}")

q1_min_x,  median_min_x , q3_min_x= np.percentile(min_xs, [25, 50, 75])
q1_max_x,  median_max_x , q3_max_x= np.percentile(max_xs, [25, 50, 75])
q1_min_y,  median_min_y , q3_min_y= np.percentile(min_ys, [25, 50, 75])
q1_max_y,  median_max_y , q3_max_y= np.percentile(max_ys, [25, 50, 75])

tol_min_x=max(tol, median_min_x - q1_min_x)
tol_max_x=max(tol, q3_max_x - median_max_x)
tol_min_y=max(tol, median_min_y - q1_min_y)
tol_max_y=max(tol, q3_max_y - median_max_y) 
...
for f, b in all_bounds.items():
    if ((b.left   <= 0.5*(median_min_x + median_max_x)) and # the tile must contain the median point of all tiles
        (b.right  >= 0.5*(median_min_x + median_max_x)) and
        (b.bottom <= 0.5*(median_min_y + median_max_y)) and
        (b.top    >= 0.5*(median_min_y + median_max_y))):
...
print(f"Continuing with {len
                         (sorted_files)} files in set S")
...
print(f"Header check complete. {len(sorted_files)} file(s) out of {len(all_bounds)} ready for HDF5 write.")
