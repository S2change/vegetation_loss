...
h5_filename = os.path.join(folder_path, 'test_6bands.h5')
...
# 1. Parse and Sort Files (Using 4bands as the reference) <<<<<<<<< 4bands as reference for sorting and metadata extraction, since they are the main bands used for CCD and we want to ensure the temporal order is based on these. We will then check spatial alignment with 2bands and filter out any files that do not match spatially.
...
sorted_files = [m['filename'] for m in file_metadata] # e.g. S2SR_image_1491651247967.tif (note the name is the same for 4bands and 2bands)

# 2. Find the Common Intersection Bounding Box

# Step 2a: Read all extents (using the 4-band folder as the spatial reference)
print("Reading extents from all 4band files...")
...
with rasterio.open(os.path.join(folder_path_4bands, sorted_files[0])) as src:  #<<<<<<<<<<<<<<<<<<<<<< 4bands as reference
    inter_window_4b = from_bounds(inter_left, inter_bottom, inter_right, inter_top, src.transform)
    win_width = int(inter_window_4b.width)
    win_height = int(inter_window_4b.height)
    total_pixels = win_width * win_height
    
    win_transform = src.window_transform(inter_window_4b)   #<<<<<<<<<<<<<<<<<<<<<< 4bands as reference for win_transform, since we will use this to extract the same pixel coordinates from both 4-band and 2-band files, ensuring they are spatially aligned. We will check header compatibility with 2bands later to ensure they match spatially.
...
with rasterio.open(os.path.join(folder_path_2bands, sorted_files[0])) as src:  #<<<<<<<<<<<<<<<<<<<<<< 2bands as reference
    inter_window_2b = from_bounds(inter_left, inter_bottom, inter_right, inter_top, src.transform)
...
if False: # Set to True to enable header check (optional but recommended for data integrity)
...
with h5py.File(h5_filename, 'w') as h5f:
    ...
    for i, filename in enumerate(sorted_files):
        ...
        with rasterio.open(path_4b) as src4, rasterio.open(path_2b) as src2:
            ...
            data4 = src4.read(window=inter_window_4b)[:, :win_height, :win_width].reshape(4, -1)
            data2 = src2.read(window=inter_window_2b)[:, :win_height, :win_width].reshape(2, -1)
