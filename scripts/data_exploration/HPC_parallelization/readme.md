**Note** The most recent overall pipeline is available at (https://github.com/S2change/vegetation_loss/blob/main/scripts/data_exploration/HPC_parallelization/general_pipeline.md)

Prompt: he processing part is to be done on HPC with 5 nodes and 96 cores/node over the  N_TS selected timestamps.  I know that each cpu has 5 GB of RAM. I guess that the idea is that each task will process a set of chips. The total number of chips can be as large as 1600 for the largest tile. There is an additional complication: I want to apply the prediction model to each chip, but also to new chips that overlap 50% with the original ones horizontally and vertically. This means that my total number of $256 \times 256 \times$ T_TS arrays to be processed will be ~4 times the original number of independent chips. This also mean that processing is not trivially parallelizable. My idea was to distribute blocks of chips, with different tasks sharing one chip-columns or one-chip row of those blocks. Can you suggest a strategy to distribute the work per tasks?
