...
CLASS_WEIGHTS=[1.0, 3.0, 10.0, 10.0, 2.0] # [1.0, 20., 50.0, 50.0, 5.0] # Adjust these based on your earlier distribution (90%, 0.23%, etc.) and experimentation; higher numbers for rarer classes to help the model learn better from the imbalanced classes in our dataset, especially since we have a very high imbalance with Class 0 being much more prevalent than the other classes; these weights can be adjusted based on experimentation and the specific distribution of classes in your training data for potentially improved performance
...
working_dir = r"C:\Users\mlc\Downloads\temp\test_tif_to_hdf5\training_data"
Train_im_pathA = os.path.join(working_dir, "before")  
Train_im_pathB = os.path.join(working_dir, "after")  
Train_lb_path = os.path.join(working_dir, "label")  

# OUtput model weights path (update this path if you want to save the trained model weights in a different location)
Train_weight_path = os.path.join(working_dir, "bacdm_weights","vchips2020_90_FS_0103101002_LR01_alpha50_")  # OUTPUT prefix to pth and txt file names; see train.py # 权重保存的路径

# 6 bands original weights path 
if True:
    Train_pretrained_path = os.path.join(working_dir, "logs", "B12118A432.pth") # used in scripts\bacdm\swin_ynet.py for loading pretrained weights for training; set to None if you don't want to use pretrained weights for training, or update this path if you have pretrained weights saved in a different location that you want to use for training
    LearningRate = 0.01
else:
    Train_pretrained_path = r"C:\Users\mlc\Downloads\temp\test_tif_to_hdf5\training_data\bacdm_weights\vchips2020_90_CE_Dice_20260408033235_99.pth"
...
Test_im_pathA = os.path.join(working_dir, "training_data", "before")
Test_im_pathB = os.path.join(working_dir, "training_data", "after")
# where predicted change maps will be saved (both as png and geotiff)
Test_det_path = os.path.join(working_dir, "training_data", "predictions") # change this suffix if you want to save predictions in a different folder
