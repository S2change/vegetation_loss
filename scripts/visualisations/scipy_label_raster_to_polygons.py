...
output_vector = "/Users/domwelsh/green_ds/Thesis/BDR_300_artigo/personal_tests/11_label_clusters_shapes_conn_8.gpkg"  # Path to output vector file
...
def create_spatial_temporal_groups(raster_array, date_range_days=0, connectivity=8):
    ...
    print(f"Applied {total_mappings} date mappings across {num_features} spatial clusters")
...
def raster_to_polygons(input_raster, output_vector, band_number=1, date_range_days=0,
    ...
    if date_range_days > 0:
        print(f"Grouping pixels within {date_range_days} days per spatial cluster...")
