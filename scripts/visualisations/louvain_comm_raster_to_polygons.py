"""
Raster to Vector Polygon Converter using Louvain Community Detection

This script converts a raster TIFF file to vector polygons with spatial-temporal clustering.

- Uses Louvain community detection algorithm (networkx + scipy KDTree) to cluster pixels
- Combines spatial connectivity and temporal tolerance using weighted graph edges
- Configurable connectivity (4-connectivity or 8-connectivity)
- Edge weights based on temporal similarity (lower difference = higher weight)

Features:
- Spatial-temporal clustering of pixels using community detection
...
resolution = 1.0  # Louvain resolution parameter: higher values -> more/smaller communities, lower values -> fewer/larger communities (default: 1.0)
...
def louvain_clustering(image, tolerance, connectivity=8, resolution=1.0):
    """
    Cluster pixels using Louvain community detection with spatial and temporal constraints.

    Args:
        image: numpy array with numeric values (e.g., days since reference)
        tolerance: maximum difference in values to consider for edge creation
        connectivity: 4 for edge-only, 8 for edge+diagonal (default: 8)
        resolution: resolution parameter for Louvain algorithm (default: 1.0)
    ...
    # Add weighted edges based on temporal similarity
    # Weight is inversely related to temporal difference
    # Higher weight = more similar temporally = stronger connection
    for i, j in pairs:
        temporal_diff = abs(values[i] - values[j])

        # Only add edge if within tolerance
        if temporal_diff <= tolerance:
            # Weight calculation: inverse of (difference + 1) to avoid division by zero
            # Pixels with identical dates get highest weight
            weight = 1.0 / (temporal_diff + 1.0)
            G.add_edge(i, j, weight=weight)

    # Apply Louvain community detection algorithm
    # This finds communities that maximize modularity
...
def create_spatial_temporal_groups(raster_array, date_range_days=0, connectivity=8, resolution=1.0):
    if date_range_days == 0:
        return raster_array, raster_array

    print(f"Starting spatial-temporal clustering (connectivity={connectivity}, date_range={date_range_days} days, resolution={resolution})...")
    ...
    start_time = time.time()
    labels, num_clusters = louvain_clustering(days_array, tolerance=date_range_days, connectivity=connectivity, resolution=resolution)
    elapsed = time.time() - start_time
    print(f"  Louvain clustering took {elapsed:.2f} seconds")
...
def raster_to_polygons(input_raster, output_vector, band_1=1, band_2=2, date_range_days=0,
                       min_area_ha=0.5, nodata_value=-9999, connectivity=8, resolution=1.0):
    ...
    if date_range_days > 0:
        raster_data, cluster_labels = create_spatial_temporal_groups(raster_data, date_range_days, connectivity, resolution)
    else:
...
def main():
    ...
    print("="*60)
    print("RASTER TO POLYGON CONVERTER - LOUVAIN ALGORITHM")
    print("="*60 + "\n")
