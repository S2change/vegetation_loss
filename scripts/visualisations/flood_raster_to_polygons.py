...
- Uses flood fill algorithm (skimage.segmentation.flood) to cluster pixels
- Combines spatial connectivity and temporal tolerance in a single pass
...
output_vector = "/Users/domwelsh/green_ds/Thesis/BDR_300_artigo/personal_tests/12_flood_first_test.gpkg"  # Path to output vector file
...
def threshold_watershed(image, tolerance, connectivity=8):
    """
    Cluster pixels using flood fill algorithm with tolerance.
    ...
    # Set connectivity parameter for flood function
    # flood uses connectivity as: 1 for 4-connectivity, 2 for 8-connectivity
    flood_connectivity = 2 if connectivity == 8 else 1

    for idx, val in np.ndenumerate(image):
        # Skip already labeled pixels and zero/nodata pixels
        if labels[idx] == -1 and val != 0:
            mask = flood(image, idx, tolerance=tolerance, connectivity=flood_connectivity)
...
def create_spatial_temporal_groups(raster_array, date_range_days=0, connectivity=8):
    """
    Group pixel values based on spatial proximity and temporal similarity using flood fill.
    ...
    # Apply threshold watershed clustering
    start_time = time.time()
    labels, num_clusters = threshold_watershed(days_array, tolerance=date_range_days, connectivity=connectivity)
    elapsed = time.time() - start_time
    print(f"  Flood fill clustering took {elapsed:.2f} seconds")
...
def raster_to_polygons(input_raster, output_vector, band_number=1, date_range_days=0,
    ...
    # Group pixels by date range if specified (using flood fill clustering)
    if date_range_days > 0:
...
def main():
    ...
    print("="*60)
    print("RASTER TO POLYGON CONVERTER - FLOOD FILL ALGORITHM")
    print("="*60 + "\n")
