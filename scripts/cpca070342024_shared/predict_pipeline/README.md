...
| `MAX_THETA` | `5` (int, days) | Clustering tuning (`USE_DATE_CLUSTERS=1` only): max gap (days) for single-link merging; clustering runs theta = 1..MAX_THETA. |
| `MAX_CLUSTER_AMPLITUDE` | `10` (int, days) | Clustering tuning (`USE_DATE_CLUSTERS=1` only): max span (days) a single cluster may cover (the amplitude cap that stops a long chain of near-dates snowballing into one cluster). |
...
| `MAX_COMPOSITE_DAYS` | `45` (int, days) | Symmetric day-window around each break date for before/after compositing. Set empty (`MAX_COMPOSITE_DAYS=`) for unbounded (any timestep before/after the target). |
| `OUTPUT_CONFIDENCE` | `1` (bool: 0\|1) | When on (default), output a per-patch change confidence (0–100): the model's softmax probability of the predicted class, averaged over the votes that passed the threshold, then averaged over each patch. Adds a `confidence` column to the final `.gpkg`/`.parquet` and a per-pixel `confidence` array to the block/tile `.npz` (255 = no detection). Requires the model's `predict_before_after_chips` to support it (`enet_16bit` does); models that don't (`bacdm`/`enet_8bit`) run labels-only and the pipeline continues — `predict_block.py` detects support from the function signature and degrades gracefully with a notice. Set `0` to force off. |
