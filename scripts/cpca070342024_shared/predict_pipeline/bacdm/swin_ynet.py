...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=256, patch_size=4, in_chans=td, embed_dim=96, norm_layer=None):
        super().__init__()
...
class SwinTransEncoder(nn.Module):
    ...
    def __init__(self, img_size=256, patch_size=4, in_chans=td, num_classes=2,
...
class SwinTransDecoder(nn.Module):
    def __init__(self, img_size=256, patch_size=4, in_chans=td, num_classes=2,
        ...
        if self.final_upsample == "expand_first":
            ...
            self.up_0 = FinalPatchExpand_X4_1(input_resolution=(64, 64), dim_scale=1, dim=128, patchsize=1)
    ...
    def forward_up_features(self, x_mid, x_downsample1, x_downsample2):
        x_upsample = []
        for inx, layer_up in enumerate(self.layers_up):
            if inx == 0:
                t1 = self.transpose(x_downsample1[3])
                ...
                hidden2 = torch.cat([t1, t2], dim=1)
                hidden = x_mid + hidden + hidden2

                x = self.transpose_verse(self.ppattention[inx](hidden))

                x = self.concat_linear0(x)
                y1 = layer_up(x)
                y2 = y1
                x = torch.cat([y1, y2], dim=2)
                x_upsample.append(self.norm[0](y1))
            else:
                t1 = self.transpose(x_downsample1[3 - inx])
                ...
                hidden = torch.cat([diff_feature, add_feature], dim=1)
                hidden2 = torch.cat([t1, t2], dim=1)
                hidden = x + hidden + hidden2
                x = self.ppattention[inx](hidden)
                x = self.transpose_verse(x)
...
# Inference-only encoder: no pretrained-weight loading on init.
# load_model() in predict.py loads the full fine-tuned checkpoint after construction.
class encoder1(nn.Module):
    def __init__(self, num_classes):
        super(encoder1, self).__init__()
        self.encoder1 = SwinTransEncoder(
            img_size=256, patch_size=4, in_chans=td, num_classes=num_classes,
